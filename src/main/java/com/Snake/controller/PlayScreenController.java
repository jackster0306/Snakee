...
public class PlayScreenController {
    ...
    @FXML
    private AnchorPane PlayPaneSky;
    @FXML
    ...
    public void initialize(){
        ...
        m_playlevel = 1;
        m_time = 0.03;
        Image foodimg = new Image(getClass().getResource(Theme.GetFoodImg()).toString());
        m_wallimg = new Image(getClass().getResource(Theme.GetWallImg()).toString());
        Image bombimg = new Image(getClass().getResource(Theme.GetBombImg()).toString());
        PlayPaneSky.setId(Theme.GetBackground());
        NameLabel.setText("Player Name: "+StartScreenController.GetPlayerName());
        m_snakeheadimg = new Image(getClass().getResource(Theme.GetSnakeHImg()).toString());
    ...
    private void mainTimeline(){
        ...
        m_timeline = new Timeline(new KeyFrame(Duration.seconds(m_time), e -> {
            ...
            if (m_intersects) {
                m_wall.removeWall();
                try {
                    if(m_snake.RemoveSnakeBody(m_hit, m_snakebody))
                            toEndScreen();
                } catch (IOException ex) {
                    throw new RuntimeException(ex);
                }
    ...
    private void SwitchLevel(){
        ...
        new MusicPlayer("src/main/resources/com/Snake/Music/level_up.mp3", false);
